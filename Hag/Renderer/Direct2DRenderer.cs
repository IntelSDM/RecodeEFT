...
namespace Hag.Renderer
{
    public class Direct2DRenderer : IDisposable
    {
        ...
        public void EndScene()
        {
          ...
          /*          long endTime = stopwatch.ElapsedMilliseconds;
                    int sleepTimePerFrame = 1000 / 500;
    ...
    //            DrawLine(x - size, y - size, x + size, y + size, stroke, color);
            //    DrawLine(x + size, y - size, x - size, y + size, stroke, color);
                ...
                DrawLine(x,y, x, y - size, stroke, color);
                DrawLine(x, y, x, y + size, stroke, color);
                DrawLine(x, y, x - size, y, stroke, color);
                DrawLine(x, y, x + size, y, stroke, color);
            }
        ...
        public void RotateSwastika(float x, float y, float size, float stroke, Direct2DColor color)
        {
            ...
            if (Math.Abs(thisTime - lastTime) >= Globals.Config.Crosshair.CrosshairAnimation)
            {
                rotationState += 0.1f;
        ...
        public int DrawText(string text, float x, float y, float fontSize, Direct2DFont font, Direct2DColor color)
        {
            sharedBrush.Color = color;
            ...
            int width = (int)(layout.Metrics.Width + modifier);
        ...
        public float DrawTextLeft(string text, float x, float y, float fontSize, Direct2DFont font, Direct2DColor color)
        {
            sharedBrush.Color = color;
            ...
            device.DrawTextLayout(new RawVector2(x - (layout.Metrics.Width + modifier), y), layout, sharedBrush, DrawTextOptions.NoSnap);
            float cache = layout.Metrics.Width + modifier;
            layout.Dispose();
            return cache;
        }
        ...
        public void DrawTextMenu(string text, float x, float y, float fontSize, float height,bool background,Direct2DFont font, Direct2DBrush brush, Direct2DBrush brush2)
        {
            var layout = new TextLayout(fontFactory, text, font, float.MaxValue, float.MaxValue);

            layout.SetFontSize(fontSize, new TextRange(0, text.Length));
            if(background)
            this.FillRectangle(x - 3, y - 3, layout.Metrics.Width + 7, height, brush2);
            device.DrawTextLayout(new RawVector2(x, y), layout, brush, DrawTextOptions.NoSnap);
        ...
        public int DrawTextCentered(string text, float x, float y, float fontSize, Direct2DFont font, Direct2DColor color)
        {
            var layout = new TextLayout(fontFactory, text, font, float.MaxValue, float.MaxValue);
            layout.SetFontSize(fontSize, new TextRange(0, text.Length));
